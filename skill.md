...
`dist/emlab.html` 变大通常不是“页面结构”，而是：

- release 模式 **Plotly.js 内联**（几 MB 是正常的）
- 每个模块的 `data_payload`（预计算网格/重建结果）被直接 JSON 内联到 `<script type="application/json" id="data-...">`

实战优化思路（优先级从高到低）：

1) **只存“不可替代的基数据”**：能在浏览器 `O(N)` 推出来的派生量不要预存。
   - 例（M05）：仅预存归一化电流 `I_n(t)`，把 `Q_n=∫I dt`、`J1_n=∫I²dt` 在前端积分现算，HTML 体积可大幅下降。
2) **砍采样/砍网格**：`t` 点数、参数网格档位直接线性影响体积。
3) **离散化参数用 `<select>`**：避免把连续 slider 逼成“高维大网格”。
4) **避免冗余数组**：同一 `t`、同一 `grid` 不要多份重复保存。

快速定位“谁在撑大 HTML”的办法：统计各 `data-...` JSON 的字节数（找最大头再动刀）。
